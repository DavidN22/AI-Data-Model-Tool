    ...
    <div className="flex flex-col h-full card overflow-hidden bg-gradient-to-b from-white to-slate-50/50">
                        ...
                        <span
                          dangerouslySetInnerHTML={{
                            __html: m.content
                              ...
                              .replace(/`([^`]*)`/g, `<code class='px-1.5 py-0.5 ${m.role === "user" ? "bg-blue-700/40" : "bg-muted"} rounded text-xs font-mono'>$1</code>`),
                          }}
                        ></span>
      ...
      {!showSchemaEditor && (
        <div className="border-t bg-white/80 backdrop-blur-sm p-3">
          <form onSubmit={handleSubmit} className="flex gap-2 items-end bg-muted/40 rounded-2xl p-2 border">
            <textarea
              value={input}
              onChange={(e) => {
